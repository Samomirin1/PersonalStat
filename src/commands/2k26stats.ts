...
const command: Command = {
  data: new SlashCommandBuilder()
    .setName("2k26stats")
    .setDescription("Show a player's archived 2K26 career stats")
  ...
  async execute(interaction) {
    ...
    if (stats.gp === 0) {
      await interaction.reply({ content: `${player.gamertag} has no recorded games from ${EDITION}.` });
      return;
    }

    await interaction.reply({ embeds: [buildStatsEmbed(player.gamertag, `${EDITION} Archive`, stats)] });
  },
